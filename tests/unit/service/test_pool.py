...
async def test_no_op_task():
    with temporary_settings({'version': 2}):
        ...
        cleared_task = asyncio.create_task(dispatcher.pool.events.work_cleared.wait())
        await dispatcher.pool.dispatch_task({'task': 'lambda: None'})
        await cleared_task
        await dispatcher.pool.shutdown()
